...
<body>
  ...
  <div class="layout">
	...
	<c:if test="${loginUser.admin == 'Y'}">
      <div id="addbutton">
        <!-- 게시글 작성 버튼 -->
        <a href='${pageContext.request.contextPath}/event/add.do?eventBbsid=${bbs.eventBbsId}'><button type="button" class="btn btn-primary">
      ...
      <table>
        ...
        <tbody>
          ...
          <c:forEach items="${pagination}" var="bbs">
            <tr>
              <td>${bbs.eventBbsNo}</td>
              <td><a href='${pageContext.request.contextPath}/event/select.do?eventBbsNo=${bbs.eventBbsNo}'>${bbs.eventBbsTitle}</a></td>
              <td>${bbs.eventBbsId}</td>
              <td>${bbs.eventBbsUser}</td>
              <td>${bbs.eventBbsRegdate}</td>
              <td>${bbs.eventBbsCnt}</td> 
            </tr>
          </c:forEach>
        </tbody>
      </table>
          ...
          <ul class="pagination">
            ...
            <li class="page-item">
              <c:choose>
                <c:when test="${currentPage > 1}">
                  <a class="page-link" href="${pageContext.request.contextPath}/event/list.do?page=${currentPage - 1}&size=${pageSize}" aria-label="Previous">
            ...
            <c:forEach begin="1" end="${totalPages}" var="i">
              <c:choose>
                ...
                <c:otherwise>
                  <li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/event/list.do?page=${i}&size=${pageSize}">${i}</a></li>
                </c:otherwise>
              </c:choose>
            </c:forEach>
            ...
            <li class="page-item">
              <c:choose>
                <c:when test="${currentPage < totalPages}">
                  <a class="page-link" href="${pageContext.request.contextPath}/event/list.do?page=${currentPage + 1}&size=${pageSize}" aria-label="Next">
