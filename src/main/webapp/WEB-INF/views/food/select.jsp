...
<script src="https://apis.openapi.sk.com/tmap/jsv2?version=1&appKey=vzu5CCmUV08tV7GpX7vumaXh19PVhTdV8sNikcdl"></script>   

</head>
<body>
    <br><br><br><br>
        <div class="form">
         <h2>${bbsinfo.foodBbsTitle}</h2> 
            <div class="container">
                  
        <table>
            <tr>
                <th>글 번호</th>
                <td>${bbsinfo.foodBbsNo}</td>
            </tr>
            <tr>
                <th>아이디</th>
                <td>${bbsinfo.foodBbsId}</td>
            </tr>
            <tr>
                <th>작성자</th>
                <td>${bbsinfo.foodBbsUser}</td>
            </tr>
            <tr>
                <th>소개글</th>
                <td style="width: 550px; height: 400px; overflow-y: auto;">
                 	<c:out value="${bbsinfo. foodBbsCon}" escapeXml="false"/>
                 </td>
            </tr>
        </table>
        <div id="carouselExampleIndicators" class="carousel slide carousel-container" data-bs-ride="carousel">
        <h2>${bbsinfo.foodBbsTitle}의 갤러리</h2>
		    ...
		    <div class="carousel-inner">
		        <c:forEach var="attach" items="${attinfo}" varStatus="status">
		            <div class="carousel-item <c:if test='${status.first}'>active</c:if>">
		           		<img src="${pageContext.request.contextPath}/food/down.do?foodAttNo=${attach.foodAttNo}" class="d-block w-100" alt="...">
		            </div>
		        </c:forEach>
				    </div>
        ...
        <div class="footer">
            <a href="${pageContext.request.contextPath}/food/list.do"><button type="button">목록으로</button></a>
            <c:if test="${bbsinfo.foodBbsId == loginUser.id || loginUser.admin == 'Y'}">
            <a href="${pageContext.request.contextPath}/food/edit.do?foodBbsNo=${bbsinfo.foodBbsNo}"><button type="button">수정하기</button></a>
            <a href="${pageContext.request.contextPath}/food/del.do?foodBbsNo=${bbsinfo.foodBbsNo}"><button type="button">삭제하기</button></a>
			</c:if>        
        </div>
</div>
		<br><br>
...
var address = '${fn:escapeXml(bbsinfo.foodBbsAddress)}';
    ...
    // 정상적으로 검색이 완료됐으면 
     if (status === kakao.maps.services.Status.OK) {
        ...
        var infowindow = new kakao.maps.InfoWindow({
            content: '<div style="width:150px;text-align:center;padding:6px 0;">${bbsinfo.foodBbsTitle}</div>'
        });
        ...
        <script type="text/javascript">
    	 ...
    	 // 댓글 저장
            document.querySelector('#repAddBtn').onclick = function() {
                ...
                fetch('${pageContext.request.contextPath}/foodreply/add.do', {
                    method: 'POST', 
                    headers: {
                    ...
                    body: new URLSearchParams({
                    	foodRepContent: repContent,
                        foodRepBbsNo: '${bbsinfo.foodBbsNo}',
                    	rating: rating
                })
                })
                .then(resp => {
         ...
         // 댓글 목록 갱신
            function refresh() {
            fetch('${pageContext.request.contextPath}/foodreply/list.do?foodRepBbsNo=${bbsinfo.foodBbsNo}', {
                method: 'GET',
            ...
            .then(resp => { //resp = httpResponse의 변수명 
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    rt.querySelector('.writer').textContent = v.foodRepWriter;
                    rt.querySelector('.date').textContent = v.foodRepDate;
                    rt.querySelector('.content').textContent = v.foodRepContent;
                    ...
                    if(v.foodRepWriter == '${loginUser.username}'){ 
                    	rt.querySelector('.del').setAttribute('data-no', v.foodRepNo);
                        rt.querySelector('.del').onclick = delReply;
                        
                        rt.querySelector('.edit').setAttribute('data-no', v.foodRepNo);
            ...
            function delReply(ev){
            	...
            	fetch('${pageContext.request.contextPath}/foodreply/del.do?foodRepNo='+no,{
            		method: 'GET',
            		headers: {'Accept':'application/json' }
            	})
            	.then(resp=>{
            ...
            function editReply(ev){
            	...
            	fetch('${pageContext.request.contextPath}/foodreply/edit.do?foodRepNo=' + no + '&foodRepContent=' + encodeURIComponent(newContent), {
            		method:'GET',
            		headers: {'Accept':'application/json' } 
            	})
            	.then(resp=>{
