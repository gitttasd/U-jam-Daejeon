...
.star.filled {
 ...
 #main {
      position: relative;
      color: white;
      background-image: url('${pageContext.request.contextPath}/bbs/down.do?attNo=238');
...
<section class="py-5 text-center container" id="main">
    <div class="row py-lg-5">
      <div class="col-lg-6 col-md-8 mx-auto">
        <h1 class="fw-light"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">대전에서 즐길거리 찾기</font></font></h1>
        <p class="lead text-body-secondary"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">대전의 모든 즐길거리를 한곳에서 확인하세요!</font></font></p>
        <p>
          <form id="searchForm" action="${pageContext.request.contextPath}/bbs/list.do" class="col-12 col-lg-auto mb-3 mb-lg-0" role="search">
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
            <c:forEach items="${pagination}" var="bbs">
            	<c:choose>
            	 <c:when test="${bbs.bbsStatus == 'Y'}">
                <div class="col">
                    <div class="card shadow-sm">
                        <c:choose>
                             <c:when test="${!empty bbs.attList}">
                                <c:if test="${!empty bbs.attList[0].attNo}">
                                    <!-- 첫 번째 첨부 파일의 attNo를 사용 -->
                                    <img src="${pageContext.request.contextPath}/bbs/down.do?attNo=${bbs.attList[0].attNo}" class="bd-placeholder-img card-img-top" width="100%" height="225" alt="Thumbnail">
                                </c:if>
                            </c:when>
                            <c:otherwise>
            ...
            <div class="card-body">
              <p class="card-text">게시글 번호: ${bbs.bbsNo}</p>
              <p class="card-text"><a href='${pageContext.request.contextPath}/bbs/select.do?bbsNo=${bbs.bbsNo}'>${bbs.bbsTitle}</a></p>
		 <p class="card-text">추천:
		    <c:choose>
              ...
              <div class="d-flex justify-content-between align-items-center">
                <small class="text-body-secondary">조회수:${bbs.bbsCnt}</small>
              </div>
            </div>
          </div>
        </div>
      ...
      <c:if test="${loginUser.admin == 'Y'}">
        <!-- 게시글 작성 버튼 -->
        <a href='${pageContext.request.contextPath}/bbs/add.do'><button type="button" class="btn btn-primary">
